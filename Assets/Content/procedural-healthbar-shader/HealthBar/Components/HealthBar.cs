...
namespace Plugins.procedural_healthbar_shader.HealthBar.Components
{
    ...
    public class HealthBar : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (Time.frameCount % 2 == 0 && !Mathf.Approximately(_oldHealthNormalized, HealthNormalized))
            {
                SetMaterialData();
        ...
        void SetMaterialData()
        {
            ...
            if (Application.isPlaying)
                _matInstance.SetFloat("_healthNormalized", HealthNormalized);
            else
            ...
            if (Application.isPlaying)
                _matInstance.SetColor(FillColor, _lowToHighHealthTransition.Evaluate(HealthNormalized));
            else
